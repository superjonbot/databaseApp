...
//added to so searchType can be passed in as an argument
const searchType = process.argv.slice(2)[0] || 'brute';
...
function makeQueriesRecursively(results, start, end, count, callback) {
    if (count) {
        ...
        //added searchType to the query string
        makeAPIRequest(`/media-segment?position=${position}&searchType=${searchType}`, res => {
